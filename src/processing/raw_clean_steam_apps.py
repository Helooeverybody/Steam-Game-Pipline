...
def clean_ratings(game):
    ...
    for region, info in ratings.items():
        ...
        formatted_regions.append(f"{region}: (required_age={age}, descriptors={descriptors})")
...
def parse_date_native(col_name):
    month_map_regex = {
        "janv.|ene.": "Jan", "févr.|febr.": "Feb", "mars|märz": "Mar", "avr.|abr.": "Apr", "mai": "May",
        "juin": "Jun", "juil.": "Jul", "août|ago.": "Aug", "sept.": "Sep", "oct.|okt.": "Oct",
        "nov.": "Nov", "déc.|dic.|dez.": "Dec"
    }
    c = F.col(col_name)
    for pattern, replacement in month_map_regex.items():
        c = F.regexp_replace(c, f"(?i){pattern}", replacement)
    return F.coalesce(
        F.to_date(c, "d MMM, yyyy"),
        F.to_date(c, "MMM d, yyyy"),
        F.to_date(c, "d MMM yyyy"),
        F.to_date(c, "MMM yyyy"),
        F.to_date(c, "yyyy")
    )

def array_clean_native(col_name):
    return F.expr(f"filter(transform({col_name}, x -> trim(x)), x -> x != '' and x is not null)")
...
full_game_schema = StructType([
    ...
    # Ratings: Map of "Region" -> Struct
    StructField("ratings", MapType(StringType(), StructType([
...
input_path = "s3a://spark-scripts/steam_apps_dataset_raw_part_01.jsonl"
...
# Explode to get rows
df = df.select(F.explode("data_map").alias("appid", "json_value"))

df_parsed = df.withColumn("game", F.from_json(F.col("json_value"), full_game_schema))
...
df_cleaned = df_parsed.select(
    ...
    # --- Text ---
    F.coalesce(clean_text_native("game.detailed_description"), F.lit("")).alias("detailed_description"),
    F.coalesce(clean_text_native("game.about_the_game"), F.lit("")).alias("about_the_game"),
    F.coalesce(clean_text_native("game.short_description"), F.lit("")).alias("short_description"),
    F.coalesce(clean_text_native("game.supported_languages"), F.lit("")).alias("supported_languages"),

    # --- Ratings ---
    F.coalesce(clean_ratings_udf(F.col("game")), F.lit("")).alias("regions_description"),

    # --- Categories/Genres ---
    F.coalesce(
        F.expr("filter(transform(coalesce(game.categories, array()), x -> trim(x.description)), x -> x is not null and x != '')"),
        F.array()
    ).alias("categories"),

    F.coalesce(
        F.expr("filter(transform(coalesce(game.genres, array()), x -> trim(x.description)), x -> x is not null and x != '')"),
        F.array()
    ).alias("genres"),
    ...
    F.coalesce(
        F.expr("transform(coalesce(game.achievements.highlighted, array()), x -> x.name)"), 
        F.array()
    ...
    # --- Price ---
    F.coalesce(F.round(F.col("game.price_overview.initial").cast("double") / 100, 2), F.lit(None)).alias("initial_price"),
    F.coalesce(F.round(F.col("game.price_overview.final").cast("double") / 100, 2), F.lit(None)).alias("final_price"),
    ...
    # --- Movies ---
    F.coalesce(
        F.expr("filter(transform(coalesce(game.movies, array()), m -> trim(m.name)), x -> x is not null and x != '')"),
        F.array()
    ).alias("movies"),
