...
def preprocess_data(df):
    ...
    return df.select(
        ...
        F.coalesce(F.col("genres"), F.array().cast("array<string>")).alias(
            "genres_arr"
        ),
        F.coalesce(F.col("categories"), F.array().cast("array<string>")).alias(
            "categories_arr"
        ),
    )
...
def build_pipeline():
    ...
    vectorizer_cats = CountVectorizer(
        inputCol="categories_arr", outputCol="cats_vec", vocabSize=50, minDF=1.0
    )
    ...
    rf = RandomForestRegressor(
        featuresCol="features", labelCol="label", numTrees=50, maxDepth=10, seed=42
    )

    return Pipeline(stages=[vectorizer_genres, vectorizer_cats, assembler, rf])
